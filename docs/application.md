...
- GitHub repository:
  ...
  ```text
  https://github.com/WinterStarAbe/DocPilot
  ```
...
- Gemini live model 報告：

  ```powershell
  node dist/src/cli.js analyze --provider gemini --model gemini-3.1-flash-lite-preview --out reports/docpilot-gemini-report.md
  ```

  ```text
  reports/docpilot-gemini-report.md
  ```
...
- CI 證據：
  ...
  GitHub Actions 會在 push 與 pull request 時執行 build、lint、test 與自舉文件分析，並上傳 `reports/docpilot-report.md` 作為 workflow artifact。
...
4. 增加 release artifact，把 mock、Gemini 與 bad-docs 報告打包成可下載成果包。
